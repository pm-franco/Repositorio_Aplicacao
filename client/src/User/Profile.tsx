...
import {ADMIN, API_BASE_URL, RESEARCHER, setHeight, setWidth} from "../Extra/Helper";
...
function Profile(prop: any) {
    ...
    useEffect(() => {
        fetch(API_BASE_URL + 'user/email/' + emailStorage, {
            method: 'GET'
        })
    ...
    useEffect(() => {
        if (roleStorage === RESEARCHER || roleStorage === ADMIN) {
            fetch(API_BASE_URL + 'artwork/user/' + emailStorage, {
                method: 'GET'
            })
                .then(response => {
            ...
            <main>
                <section>
                    <header>
                        ...
                        <Link to={"/edit_person/" + emailStorage} state={person && {
                            name: person["name"],
